...
@Component({
  selector: 'kt-import-chapter',
  templateUrl: './import-chapter.component.html',
  styleUrls: ['./import-chapter.component.scss']
})
export class ImportChapterComponent implements OnInit {

  ELEMENT_DATA = []
  displayedColumns: string[] = ['select', 'name', 'board', 'class', 'syllabus', 'subject'];
  ...
  getGlobalChapterData() {
    this.apiService.getGlobalChapters().subscribe((response: any) => {
  ...
  submit() {
    ...
    this.selection.selected.forEach(selected => {
      ...
      this.updateChapter(selected)
    });
  }

  updateChapter(chapter) {
    this.apiService.addChapter(chapter, this.repository[0].id, chapter.name).subscribe((response: any) => {
      this.index++;
      Swal.fire('Success', 'Chapter Added', 'success').then(() => location.reload());
