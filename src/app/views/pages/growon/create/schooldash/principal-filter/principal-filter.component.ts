...
@Component({
  selector: 'kt-principal-filter',
  templateUrl: './principal-filter.component.html',
  styleUrls: ['./principal-filter.component.scss']
})
export class PrincipalFilterComponent implements OnInit {
  ...
  openDialog(element?: any) {
    const dialogRef = this.dialog.open(PrincipalDialogComponent, {
      hasBackdrop: true,
      data: {
  ...
  getStats() {
    this.api.getStats(this.schoolId).subscribe((res: any) => {
      this.data = res.body.data.principal
  ...
  ExportTOExcel() {
    ...
    XLSX.writeFile(wb, `Principals.xlsx`);
  }
