...
@Component({
  selector: 'kt-global-admin-edit',
  templateUrl: './global-admin-edit.component.html',
  styleUrls: ['./global-admin-edit.component.scss']
})
export class GlobalAdminEditComponent implements OnInit {
  ...
  getAllUsers() {
    ...
    this.apiService.getAllUsers(data).subscribe((response: any) => {
      ...
      console.log(response.body.data.filter(row => row.profile_type.role_name == 'admin'))

      this.user = response.body.data.filter(row => {
        return row.profile_type.role_name == 'admin'
      })
